  ...
  */
  ...
  void VescDriver::speedCallback(const std_msgs::Float64::ConstPtr &speed)
  {
    if (driver_mode_ = MODE_OPERATING)
    {
      ...
      ROS_INFO("speed->data : %f", speed->data);

      /*
      if( ((speed->data/32600) < 0.05) && ( speed->data/32600 > -0.05) )
      {
    ...
    // ROS_INFO("0");
      ...
      {
        vesc_.setDutyCycle(duty_cycle_limit_.clip((speed->data)/32600));
    // ROS_INFO("%f", duty_cycle_limit_.clip((speed->data)/32600));
      }
      */
    }
  }
                                         ...
                                         const boost::optional<double> &max_upper) : name(str)
  {
    ...
    if (nh.getParam(name + "_min", param_min))
    {
      ...
      {
        lower = *min_lower;
        ROS_WARN_STREAM("Parameter " << name << "_min (" << param_min << ") is less than the feasible minimum (" << *min_lower << ").");
      }
      else if (max_upper && param_min > *max_upper)
      {
        lower = *max_upper;
        ROS_WARN_STREAM("Parameter " << name << "_min (" << param_min << ") is greater than the feasible maximum (" << *max_upper << ").");
      }
      else
      {
        lower = param_min;
    ...
    if (nh.getParam(name + "_max", param_max))
    {
      ...
      {
        upper = *min_lower;
        ROS_WARN_STREAM("Parameter " << name << "_max (" << param_max << ") is less than the feasible minimum (" << *min_lower << ").");
      }
      else if (max_upper && param_max > *max_upper)
      {
        upper = *max_upper;
        ROS_WARN_STREAM("Parameter " << name << "_max (" << param_max << ") is greater than the feasible maximum (" << *max_upper << ").");
      }
      else
      {
        upper = param_max;
